...
static DispenserState_t CurrentState;
...
ES_Event_t RunDispenserToBucketFSM( ES_Event_t CurrentEvent )
{
   ...
   DispenserState_t  NextState = CurrentState;
   ES_Event_t EntryEventKind = { ES_ENTRY, 0 };// default to normal entry to new state
   ES_Event_t ReturnEvent = CurrentEvent; // assume we are not consuming event

   switch ( CurrentState )
   {
        /**************** Go backwards directly until tape ****************/
        case Bwd_DB:
                ...
                (CurrentEvent.EventType == ES_ENTRY_HISTORY))
            {
                ...
                CurrentEvent.EventParam = DRIVE_BACKWARD_30;
...
//                (CurrentEvent.EventType == ES_ENTRY_HISTORY))
//            {
//                DB_printf("Bwd_To_Bucket\n");
//            }
//            else if (CurrentEvent.EventType == ES_EXIT)
//            {
//                // Stop drive motors if needed
//            }
//            else
//            {
//                switch (CurrentEvent.EventType)
...
//                            // stop robot
//                            DB_printf("Go to dispense balls \n");
//                            CurrentEvent.EventType = ES_LEADER_SEND;
//                            CurrentEvent.EventParam = STOP;
//                            PostSPILeaderService(CurrentEvent);
//                            
...
//                        break;
        ...
        case DispenseBalls:
                ...
                (CurrentEvent.EventType == ES_ENTRY_HISTORY))
            {
                // init dispense timer
                CurrentEvent.EventType = ES_LEADER_SEND;
                CurrentEvent.EventParam = DRIVE_BACKWARD_10; // Probably do distance instead of timer
                PostSPILeaderService(CurrentEvent);
            ...
            {
                CurrentEvent.EventType = ES_LEADER_SEND;
                CurrentEvent.EventParam = DRIVE_FORWARD_10; // Probably do distance instead of timer
                PostSPILeaderService(CurrentEvent);                
            }
            else
            {
    ...
    if (MakeTransition == true)
    {
       ...
       RunDispenserToBucketFSM(CurrentEvent);

       CurrentState = NextState; //Modify state variable

       //   Execute entry function for new state
       // this defaults to ES_ENTRY
       RunDispenserToBucketFSM(EntryEventKind);
     }
     return(ReturnEvent);
}
/****************************************************************************

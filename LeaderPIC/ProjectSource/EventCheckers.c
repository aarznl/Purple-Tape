...
bool Check4ButtonDown(void)
{
    ...
    bool CurrentState = PORTBbits.RB8;

    if (CurrentState == 1 && LastState == 0) {
        ES_Event_t ButtonEvent;
        ButtonEvent.EventType = START;
        PostMasterSM(ButtonEvent);
...
bool Check4SwitchPushed(void)
{
    TRISBbits.TRISB10 = 1;
    static bool LastState = 0;
    bool CurrentState = PORTBbits.RB10;

    if (CurrentState == 1 && LastState == 0) {
        ES_Event_t ButtonEvent;
        ButtonEvent.EventType = SWITCH_ON;
        PostMasterSM(ButtonEvent);
        LastState = CurrentState;
        return true;
    }
